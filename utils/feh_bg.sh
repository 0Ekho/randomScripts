...
if [ "$(cat "$location/fb_stop")" -eq "0" ]; then
    ...
    # feh option
    feh --bg-fill "${current[0]}" "${current[1]}" "${current[2]}" \
"${current[3]}"
    # switched to xfce and feh doesn't seem to work on 4.12
    #xfconf-query -c xfce4-desktop -p \
    #/backdrop/screen0/monitor0/workspace0/last-image -s "${current[0]}"
    #xfconf-query -c xfce4-desktop -p \
    #/backdrop/screen0/monitor1/workspace0/last-image -s "${current[1]}"
    #xfconf-query -c xfce4-desktop -p \
    #/backdrop/screen0/monitor2/workspace0/last-image -s "${current[2]}"
    #xfconf-query -c xfce4-desktop -p \
    #/backdrop/screen0/monitor3/workspace0/last-image -s "${current[3]}"
    printf "%s\n%s\n%s\n%s\n%s" \
